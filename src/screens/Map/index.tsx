...
function MapScreen() {
  ...
  const increase = useMemo(() => {
    return {
      name: artist.recordLabelId
        ? recordLabels[artist.recordLabelId - 1].name
        : "",
      value: artist.recordLabelId
        ? recordLabels[artist.recordLabelId - 1].concertIncrease
        : 0,
    };
  }, [artist]);
    ...
    <>
      <S.Menu>
        <S.MenuContainer>
          ...
          <S.RightContent>
            <button
              onClick={() => setIsModalContractOpen(true)}
              className="btn-menu"
            >
              CONTRACTS
            </button>
